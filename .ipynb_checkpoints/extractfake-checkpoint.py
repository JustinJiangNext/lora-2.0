...
def getFakest(ratingList):
    ratingMap = {ratingList[i]: i + 1 for i in range(0, len(ratingList))}
    sortedMap = collections.OrderedDict(sorted(ratingMap.items(), reverse=True))
    res = []
...
if __name__ == '__main__':
    ...
    print(fetchDevice())
    model.load_state_dict(torch.load('weights.pth', map_location=fetchDevice()))
    model.eval()
